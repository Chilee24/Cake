...
class CosineDistillationLoss(nn.Module):
    def __init__(self, alpha=1.0):
        super(CosineDistillationLoss, self).__init__()
        self.alpha = alpha
    ...
    def forward(self, student_feat, teacher_feat):
        """
        Args:
            student_feat: (B, C, T, H, W)
            teacher_feat: (B, C, T, H, W)
        """
        # 1. Làm phẳng Feature Map thành 1 vector duy nhất cho mỗi mẫu trong Batch
        # (B, C, T, H, W) -> (B, C*T*H*W)
        s_flat = student_feat.view(student_feat.size(0), -1)
        t_flat = teacher_feat.view(teacher_feat.size(0), -1)
        
        # 2. Tạo Target: Ta muốn chúng GIỐNG NHAU => Target = 1
        target = torch.ones(s_flat.size(0)).to(s_flat.device)
        
        # 3. Tính Loss
        # Loss = 1 - cos(theta)
        loss = self.criterion(s_flat, t_flat, target)
        
        return loss * self.alpha

# ==============================================================================
# 2. DATASET (Updated with Stride)
# ==============================================================================
class RGBVideoDataset(Dataset):
    def __init__(self, video_list_file, root_dir, clip_len=13, crop_size=224, stride=6, mode='train'):
        ...
        self.stride = stride # [NEW] Stride tham số hóa
        self.mode = mode
        self.samples = []
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Tính span thực tế: (13-1)*stride + 1
...
# ==============================================================================
# 3. VALIDATION (SỬA ĐỔI: Tách Loss để theo dõi Feature Quality)
# ==============================================================================
def validate(val_loader, student, teacher, criterion_cls, criterion_distill, device, normalizer):
    losses = AverageMeter()
    # Thêm meter riêng cho Cosine Similarity
    cos_meter = AverageMeter() 
    distill_meter = AverageMeter()
    
    # Acc vẫn đo để tham khảo (dù biết sẽ thấp)
    ...
    with torch.no_grad():
        ...
        for i, (raw_clip, labels) in enumerate(pbar):
            ...
            cos_meter.update(cos_sim.item(), raw_clip.size(0))

            # 3. Tính Cls Metrics (Tham khảo)
            # Không cộng l_flow vào losses tổng để tránh nhiễu
            l_rgb = criterion_cls(rgb_logits, labels)
            
            # CHỈ TÍNH DISTILL LOSS VÀO LOSS TỔNG (để đánh giá model tốt/xấu)
            # Nếu thích có thể cộng thêm l_rgb, nhưng l_flow thì KHÔNG.
            loss = l_distill 
            ...
            pbar.set_postfix(Dis=f"{distill_meter.avg:.4f}", Cos=f"{cos_meter.avg:.3f}")
            
    logging.info(f" * Val: Distill {distill_meter.avg:.4f} | Cos {cos_meter.avg:.3f} | Flow_Acc {acc_flow.avg:.2f}%")
    
    # TRẢ VỀ COSINE SIMILARITY ĐỂ LÀM TIÊU CHÍ CHỌN BEST MODEL
    return cos_meter.avg
...
def main(args):
    ...
    # Teacher cũng dùng Stride 6 mặc định
    ...
    # [THÊM ĐOẠN NÀY]
    elif args.distill_type == 'cosine':
        logging.info(f"👉 Using Direct Cosine Similarity Loss with alpha={args.alpha_distill}")
        criterion_distill = CosineDistillationLoss(alpha=args.alpha_distill).to(device)
        ...
        class MSEWrapper(nn.Module):
            def __init__(self, alpha): 
                super().__init__(); 
                self.alpha = alpha; self.mse = nn.MSELoss()
            def forward(self, s, t):
                return self.mse(s, t) * self.alpha
    ...
    # [QUAN TRỌNG] Truyền stride vào dataset
    train_ds = RGBVideoDataset(args.train_list, args.train_root, clip_len=args.clip_len, stride=args.stride, mode='train')
    ...
    if args.val_list:
        ...
        val_ds = RGBVideoDataset(args.val_list, val_root, clip_len=args.clip_len, stride=args.stride, mode='val')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.freeze_rgb:
            student.blocks.eval()
            student.head.eval()
            logging.info("❄️ FREEZING RGB BACKBONE & HEAD (Chế độ tập trung học Flow) ❄️")
            for param in student.blocks.parameters():
                param.requires_grad = False
            for param in student.head.parameters():
                param.requires_grad = False
        ...
        for i, (raw_clip, labels) in enumerate(pbar):
            ...
            with autocast(device_type='cuda', enabled=True):
                ...
                #     a_rgb = accuracy(rgb_logits[valid_idx], labels[valid_idx], topk=(1,))
                #     acc_r_m.update(a_rgb[0].item(), valid_idx.sum().item())
                #     loss_r_m.update(l_rgb.item() if not args.freeze_rgb else 0.0)
                    
                #     l_flow = criterion_cls(flow_logits[valid_idx], labels[valid_idx])
                #     a_flow = accuracy(flow_logits[valid_idx], labels[valid_idx], topk=(1,))
                #     loss_f_m.update(l_flow.item())
        ...
        val_metric = 0.0
        if val_loader:
            val_metric = validate(val_loader, student, teacher, criterion_cls, criterion_distill, device, normalizer)
            
        # Logic lưu Best Model dựa trên Cosine (Càng cao càng tốt)
        is_best = val_metric > best_metric
        best_metric = max(val_metric, best_metric)
        
        if is_best:
            logging.info(f"⭐️ New Best Model! Cosine: {val_metric:.4f}")

        save_dict = {
            'epoch': epoch + 1,
            'state_dict': student.state_dict(),
            'best_metric': best_metric, # Lưu key mới
